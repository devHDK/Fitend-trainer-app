...
import 'package:fitend_trainer_app/thread/component/comment_cell.dart';
import 'package:fitend_trainer_app/thread/component/link_preview.dart';
import 'package:fitend_trainer_app/thread/component/preview_image.dart';
import 'package:fitend_trainer_app/thread/component/preview_video_thumbnail.dart';
import 'package:fitend_trainer_app/thread/component/profile_image.dart';
import 'package:fitend_trainer_app/thread/component/thread_detail.dart';
...
import 'package:fitend_trainer_app/thread/model/threads/thread_comment_edit_model.dart';
...
class _ThreadDetailScreenState extends ConsumerState<ThreadDetailScreen>
  ...
  Widget build(BuildContext context) {
    final family =
        ThreadFamilyModel(threadId: widget.threadId, user: widget.user);
    final state = ref.watch(threadDetailProvider(family));
    final commentState = ref.watch(commentCreateProvider(family));
    final trainerState = ref.watch(getMeProvider);

    if (state is ThreadModelLoading || trainerState is TrainerModelLoading) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Stack(
                        children: [
                          ...
                          if (commentModel.trainer != null &&
                              model.trainer.id == commentModel.trainer!.id &&
                              edittingCommentId == -1 &&
                              ...
                              child: InkWell(
                                onTap: () => DialogWidgets.editBottomModal(
                                  ...
                                  delete: () async {
                                    ...
                                    await ref
                                        .read(threadDetailProvider(family)
                                            .notifier)
                                  ...
                                  edit: () async {
                                    ...
                                    ref
                                        .read(commentCreateProvider(family)
                                            .notifier)
                                        .updateFromEditModel(
                  ...
                  : Row(
                      children: [
                        InkWell(
                          onTap: () async {
                            await ref
                                .read(commentCreateProvider(family).notifier)
                                .pickCamera(context);
                          },
                          child: Padding(
  ...
  TextFormField _commentTextFormField(ThreadFamilyModel family) {
    ...
    return TextFormField(
      autocorrect: false,
      onChanged: (value) {
        ref.read(commentCreateProvider(family).notifier).updateContent(value);
        if (mounted) {
          setState(() {
