...
class _CommentCellState extends ConsumerState<CommentCell> {
  ...
  Widget build(BuildContext context) {
    ...
    final userModel = state as TrainerModel;
  ...
  List<Widget> _buildEmojiButtons(TrainerModel trainerModel) {
    ...
    emojiCounts.forEach((key, value) {
      emojiButtons.add(EmojiButton(
        ...
        isSelected: widget.emojis.indexWhere((e) {
              return e.emoji == key && e.trainerId == trainerModel.trainer.id;
            }) >
            -1,
        onTap: () async {
          if (mounted) {
            final family = ThreadFamilyModel(
                threadId: widget.threadId, user: widget.refUser);
            await ref
                .read(threadDetailProvider(family).notifier)
                .updateCommentEmoji(
                    widget.commentId, trainerModel.trainer.id, key);
          }
        },
      ));
    });
  ...
  EmojiButton _defaultEmojiButton(
      BuildContext context, TrainerModel trainerModel) {
    return EmojiButton(
      onTap: () {
          ...
          onEmojiSelect: (category, emoji) async {
            if (emoji != null) {
              ...
              await ref
                  .read(threadDetailProvider(family).notifier)
                  .updateCommentEmoji(
                      widget.commentId, trainerModel.trainer.id, emoji.emoji);
            }
