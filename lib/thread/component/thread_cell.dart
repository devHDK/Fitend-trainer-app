...
import 'package:fitend_trainer_app/thread/component/emoji_button.dart';
import 'package:fitend_trainer_app/thread/component/link_preview.dart';
import 'package:fitend_trainer_app/thread/component/network_video_player_mini.dart';
import 'package:fitend_trainer_app/thread/component/preview_image_network.dart';
...
import 'package:fitend_trainer_app/thread/model/common/thread_workout_info_model.dart';
...
import 'package:fitend_trainer_app/thread/model/threads/thread_edit_model.dart';
...
class _ThreadCellState extends ConsumerState<ThreadCell> {
  ...
  Widget build(BuildContext context) {
    ...
    final trainerModel = state as TrainerModel;
    final family = ThreadFamilyModel(threadId: widget.id, user: widget.user);
    ...
    if (widget.emojis != null && widget.emojis!.isNotEmpty) {
      emojiButtons = _buildEmojiButtons(family);
    }

    emojiButtons.add(
      _defualtEmojiButton(context, family),
    );
    ...
    return Stack(
      children: [
        ...
        if (widget.writerType == 'trainer' &&
            widget.trainer.id == trainerModel.trainer.id &&
            ...
            child: InkWell(
              onTap: () => DialogWidgets.editBottomModal(
                ...
                delete: () async {
                  ...
                  final family =
                      ThreadFamilyModel(threadId: widget.id, user: widget.user);
                ...
                edit: () {
                  ...
                  Navigator.of(context)
                    ...
                    if (value is bool && value) {
                      await ref
                          .read(threadCreateProvider(widget.user).notifier)
                          .init();
                    }
                  });
                },
  ...
  List<Widget> _buildEmojiButtons(ThreadFamilyModel family) {
  ...
  EmojiButton _defualtEmojiButton(
      BuildContext context, ThreadFamilyModel family) {
    return EmojiButton(
      onTap: () {
