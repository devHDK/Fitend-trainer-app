...
import 'package:fitend_trainer_app/thread/model/files/file_upload_request_model.dart';
...
final threadCreateProvider = StateNotifierProvider.family<
    ThreadCreateStateNotifier, ThreadCreateTempModel, ThreadUser>((ref, user) {
  ...
  final threadListState = ref.watch(threadProvider(user).notifier);
  ...
  Future<void> pickCamera(BuildContext context) async {
    try {
      Navigator.of(context).push(
        CupertinoPageRoute(
          builder: (context) => CameraScreen(user: user),
        ),
      );
    } catch (e) {
      debugPrint(e.toString());
